...
run_inference() {
    ...
    # -------------------------------
    # Handle separable / non-separable
    # -------------------------------
    SEP_FLAG=""
    SEP_SUFFIX=""
    if [[ "$EXP" == *"_nonsep"* ]]; then
        SEP_FLAG="--no-separable"
        SEP_SUFFIX="_nonsep"
        echo "Detected separable = False"
    else
        echo "Detected separable = True"
    fi
    ...
    # Run inference
    # -------------------------------
    echo "Launching inference on $EXP (separable=${SEP_FLAG:+False}${SEP_FLAG:+'True'}) ..."
...
# run_inference "exp_10_16_warped_64/candlelight_1" "dataset_with_garment_bigface_100"
...
# run_inference "exp_10_16_warped_128_debug_nonsep/noon_sunlight_1" "dataset_with_garment_bigface_100"
# run_inference "exp_10_16_warped_128_debug_nonsep/candlelight_1" "dataset_with_garment_bigface_100"