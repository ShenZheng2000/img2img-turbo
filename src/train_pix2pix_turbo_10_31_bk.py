...
from warp_utils.warp_pipeline import get_face_app, warp_batch, unwarp_batch


def main(args):

    # ✅ Initialize face detector once for warping
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                # ✅ warp and unwarp here
                x_src_warped, warp_grids = warp_batch(x_src, face_app, args.bw)
                x_tgt_pred = net_pix2pix(x_src_warped, prompt_tokens=batch["input_ids"], deterministic=True)
                x_tgt_pred = unwarp_batch(x_tgt_pred, warp_grids)
